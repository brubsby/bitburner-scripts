...
/**
 * THE LOCK LIVES ON HOME, AND EVERY HOST CAN REACH HOME. ns.read/ns.write are
 * server-local, but ns.scp copies to and from home from anywhere, so a
 * holder off home reads the lock by pulling home's copy first and writes it
 * by pushing its copy after. Until 2026-09-19 an off-home caller was simply
 * refused — and boot.js places homeup.js "anywhere", so in BitNode 2 it sat
 * on foodnstuff refusing every pass while home stayed at 32GB with $3m in
 * hand: the fixed point the tier-8 entry exists to break. The race window is
 * one scp round-trip between two UI drivers, which never both exist below the
 * home tier where watchdog.js runs the same script as a job.
 */
// The transport is INJECTED by the off-home caller (homeup.js), because the
// copy call is a priced identifier and naming it here would charge every
// importer — cmd.js and upkeep.js on home, whose tier budgets have no room
// for it. On home both hooks are no-ops and nothing changes.
let transport = { pull: () => {}, push: () => {} }
/** Off-home callers install `{ pull(ns), push(ns) }` — see homeup.js. */
export function setLockTransport(io) {
  transport = { pull: io?.pull ?? (() => {}), push: io?.push ?? (() => {}) }
}
...
function pull(ns) {
  try {
    ...
    /* no lock on home: a stale local copy would read as held, so drop it */
    try {
      ns.rm(LOCK)
    } catch {
