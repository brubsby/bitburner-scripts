...
export async function main(ns) {
  ...
  try {
    ...
    // screen. Wait for the bridge to finish before touching anything.
    //
    // Wait on in.txt OR busy.txt, never busy.txt alone. The bridge POLLS, so
    // between a caller writing /cmd/in.txt and cmd.js picking it up there is a
    // window in which NEITHER file exists — and waiting on busy.txt alone reads
    // that window as "finished", proceeds, and gets Focus clicked out from
    // under it by a batch that had not started yet. autobuy.js:76 and
    // backdoor.js:132 already test both; this was the one that did not.
    const bridgeBusy = () => ns.fileExists('/cmd/in.txt', 'home') || ns.fileExists('/cmd/busy.txt', 'home')
    for (let waited = 0; bridgeBusy() && waited < 30000; waited += 300) {
      await ns.sleep(300)
    }
    await ns.sleep(800)
