...
// dist/go-cheat.js ONLY for a profile where sfgate.canUseGoCheat is true; in
// every other regime the file does not exist, so it cannot be run by accident
// and nothing has to gate it at runtime.
//
// The body is unchanged from the handwritten go-cheat.js: it was already a
// single-capability module, which is the shape this whole prototype is trying
// to reach everywhere else. It is priced by the RAM gate against its original
// and comes out identical, which is the result to want — the compiler should be
// a no-op on code that was already in the right shape.
