...
// ONE OF two capability modules and writes tools/compile/dist/go.js. The
// originals at the repo root are untouched.
//
//   caps/go-cheat.js  ->  src/caps/go-cheat-on.js    save can call ns.go.cheat.*
//                    ->  src/caps/go-cheat-off.js   it cannot
//
// There is no build-time constant in this file and no `if (SF14)` anywhere in
// it. The capability difference is a whole module, so it is expressed as a
// module, and the build is plain `esbuild --bundle` with tree-shaking and no
// minify flags of any kind. The output keeps its newlines, its identifiers and
// its comments; see NOTES-compile.md on debuggability.
...
import { cheatAvailable, requestCheat } from 'caps/go-cheat.js'
...
export async function main(ns) {
  ...
  // COMPILED: the 1GB ns.getResetInfo() probe is gone. canUseGoCheat is a pure
  // function of Source-File state, and Source-File state cannot change without
  // a BitNode reset, which kills every running script — so it was already a
  // constant for the life of this process. The build evaluates it once, through
  // sfgate.js, and links the answer.
  //
  // What remains a runtime question is whether go-cheat.js is actually
  // DEPLOYED, which is not a capability and can genuinely be false. That check
  // lives in caps/go-cheat-on.js and costs 0.1GB there, and nothing here.
    ...
    try {
      ...
      while (!done && guard++ < 4000) {
        ...
        if (!cheated && ranked && ranked.length >= 2 && guard > 4) {
          cheated = true
          cheatsTried++
          // The exec, the wait and the "is the helper deployed" check all live
          // in caps/go-cheat.js. In an off build requestCheat references no ns
          // function at all, so ns.exec (1.3GB) and ns.isRunning (0.1GB) are
          // not in the bundle and not on the bill. The POLICY — one cheat per
          // game, only once the position has shape, never a second — stays
          // here, in one copy, in both builds. That single copy is what the
          // handwritten go.js / go-cheat.js split could not give us.
          if (await requestCheat(ns, SETTINGS.cheat, ranked[0].x, ranked[0].y, ranked[1].x, ranked[1].y)) {
            // No return value needed beyond "a turn was consumed": every cheat
            // acts on the board, which the next iteration re-reads.
