...
const FIXTURE = path.join(HERE, "fixture-augs.json");
...
/* ------------------------------------------------------------------ AP11 --- */
// A donatable faction has no rep wall, only a price — fixed, outside the 1.9
// exponent. Before this, planPurchases skipped every rep-short offer with
// why:'reputation'; past 150 favour that wall is a dollar figure
// (donation.ts:8) the caller now attaches as `donationCost`.
function ap11() {
  const c = new Check("AP11", "donation costs open rep walls: fixed, budget-bound, outside the 1.9 exponent");
  const mk = (over = {}) => ({
    name: "Big", faction: "CyberSec", baseCost: 100e6, repReq: 200e3, factionRep: 10e3,
    mults: { hacking: 1.5 }, prereqs: [], ...over,
  });
  const base = { r: 1.9, nodeMoneyMult: 1, owned: [], soaOwned: 0 };

  c.examined(1);
  const walled = planPurchases({ ...base, offers: [mk()], money: 1e12 });
  if (walled.buy.length !== 0 || !walled.skipped.some((x) => x.why === "reputation")) {
    c.fail("a rep-short offer with no donationCost must stay skipped");
  }

  c.examined(1);
  const open = planPurchases({ ...base, offers: [mk({ donationCost: 250e6 })], money: 1e12 });
  if (open.buy.length !== 1) c.fail("a donatable rep-short offer must be plannable");
  else {
    const b = open.buy[0];
    if (Math.abs((b.donation ?? 0) - 250e6) > 1e-6) c.fail(`the buy row must carry the donation, got ${b.donation}`);
    if (Math.abs(b.price - (100e6 + 250e6)) > 1e-6) c.fail(`price must be intrinsic + donation, got ${b.price}`);
    if (Math.abs(open.totalCost - 350e6) > 1e-6) c.fail(`totalCost must include the donation, got ${open.totalCost}`);
  }

  c.examined(1);
  // The donation is OUTSIDE the exponent: with a pricier aug also bought, the
  // donation-aug's AUG price scales by 1.9^rank but the donation term does not.
  const two = planPurchases({
    ...base,
    offers: [mk({ donationCost: 250e6 }), mk({ name: "Pricier", faction: "NiteSec", baseCost: 500e6, repReq: 0, factionRep: 1, mults: { hacking: 1.4 } })],
    money: 1e12,
  });
  if (two.buy.length !== 2) c.fail(`both should be bought at $1t, got ${two.buy.length}`);
  else {
    const donated = two.buy.find((b) => b.donation);
    const expect = 100e6 * Math.pow(1.9, donated.rank) + 250e6;
    if (Math.abs(donated.price - expect) > 1e-3) c.fail(`donation must not scale with rank: price ${donated.price}, expected ${expect}`);
  }

  c.examined(1);
  // The budget binds on the SUM.
  if (planPurchases({ ...base, offers: [mk({ donationCost: 250e6 })], money: 300e6 }).buy.length !== 0) {
    c.fail("intrinsic+donation over budget must not be taken");
  }
  if (planPurchases({ ...base, offers: [mk({ donationCost: 250e6 })], money: 351e6 }).buy.length !== 1) {
    c.fail("intrinsic+donation within budget must be taken");
  }
  return c;
}

/* ------------------------------------------------------------------ AP12 --- */
// The endgame objective: Daedalus tickets and the Red Pill. Both were
// invisible to the M-objective — tickets by the zero-value rule (a $10M
// ticket sat unbought next to $34b, live), the Red Pill because m=1 makes it
// "strictly dominated by not taking it" per an objective that cannot see the
// finish line.
function ap12() {
  const c = new Check("AP12", "tickets sweep leftover budget, the Red Pill is never no-value'd, M stays honest");
  const base = { r: 1.9, nodeMoneyMult: 1, owned: [], soaOwned: 0 };
  const real = { name: "Real", faction: "F", baseCost: 1e9, repReq: 0, factionRep: 1, mults: { hacking: 1.5 }, prereqs: [] };
  const junk = (n, cost) => ({ name: n, faction: "F", baseCost: cost, repReq: 0, factionRep: 1, mults: {}, prereqs: [] });

  c.examined(1);
  // No ticketsWanted: junk stays skipped as before — the sweep is opt-in.
  const off = planPurchases({ ...base, offers: [real, junk("J1", 10e6)], money: 1e12 });
  if (off.buy.some((b) => b.kind === "ticket")) c.fail("no ticketsWanted must mean no tickets");
  if (!off.skipped.some((x) => x.name === "J1" && x.why === "no-value")) c.fail("junk without the endgame objective stays no-value");

  c.examined(1);
  // With it: junk appends AFTER the M-plan, at the higher rank, within budget.
  const on = planPurchases({ ...base, offers: [real, junk("J1", 10e6), junk("J2", 45e6)], money: 1e12, ticketsWanted: 5 });
  const t = on.buy.filter((b) => b.kind === "ticket");
  if (t.length !== 2) c.fail(`both junk augs are tickets at $1t, got ${t.length}`);
  else {
    // ORDERING IS THE DP'S, NOT THE SWEEP'S, and that is the fix rather than a
    // regression. Tickets used to be appended after the plan, cheapest first,
    // spending "only what remains" — and against NeuroFlux, which escalates
    // 2.166x per level and scales to absorb any budget, nothing ever remained.
    // Live cost in BitNode 1: four of five consecutive installs added ZERO
    // distinct augmentations (17 -> 18 -> 18 -> 21 -> 21 -> 21) while each life
    // banked ln(M) 0.59-0.95 in NeuroFlux and the count gate never moved.
    //
    // So tickets now enter the DP with a dominance value and are placed by the
    // rearrangement inequality — descending intrinsic price — which is strictly
    // CHEAPER than appending cheapest-first. Asserting the old order would pin
    // the more expensive arrangement. What must hold is the ECONOMICS:
    // every ticket carries its rank escalation, and real value still wins its
    // own place on merit.
    const real0 = on.buy.find((b) => b.name === "Real");
    if (!real0 || real0.kind === "ticket") c.fail("a genuine multiplier aug must be bought on merit, not relabelled a ticket");
    for (const x of t) {
      const base0 = x.name === "J1" ? 10e6 : 45e6;
      if (Math.abs(x.price - base0 * Math.pow(1.9, x.rank)) > 1) c.fail(`ticket price must carry 1.9^rank: ${x.name} ${x.price}`);
    }
    // Descending intrinsic is the cost-minimising order (AP1).
    const costs = t.map((x) => (x.name === "J1" ? 10e6 : 45e6));
    if (costs.length === 2 && costs[0] < costs[1]) c.fail("tickets must be placed in the cost-minimising order, like every other purchase");
  }
  c.examined(1);
  // ticketsWanted caps the sweep; budget caps it too.
  const capped = planPurchases({ ...base, offers: [junk("J1", 10e6), junk("J2", 45e6), junk("J3", 60e6)], money: 1e12, ticketsWanted: 1 });
  if (capped.buy.filter((b) => b.kind === "ticket").length !== 1) c.fail(`ticketsWanted must cap how many tickets are bought, got ${capped.buy.filter((b) => b.kind === "ticket").length}`);
  const broke = planPurchases({ ...base, offers: [junk("J1", 10e6)], money: 5e6, ticketsWanted: 5 });
  if (broke.buy.length !== 0) c.fail("a ticket over budget is not bought");
  c.examined(1);
  // Rep-short junk with NO donation is not a ticket.
  const walled = planPurchases({ ...base, offers: [{ ...junk("J1", 10e6), repReq: 5e3, factionRep: 0 }], money: 1e12, ticketsWanted: 5 });
  if (walled.buy.length !== 0) c.fail("rep-short junk with no donation must not sweep in");
  c.examined(1);
  // But rep-short junk WITH a donationCost (past 150 favour) IS a ticket —
  // the Daedalus count gate must close by dollars, not only by rep rebuild.
  const donatable = planPurchases({ ...base, offers: [{ ...junk("J1", 10e6), repReq: 5e3, factionRep: 0, donationCost: 4e9 }], money: 1e12, ticketsWanted: 5 });
  const dt = donatable.buy.find((b) => b.name === "J1" && b.kind === "ticket");
  if (!dt) c.fail("a donatable rep-short aug must sweep in as a ticket");
  else if (Math.abs(dt.donation - 4e9) > 1 || Math.abs(dt.price - (10e6 * Math.pow(1.9, dt.rank) + 4e9)) > 1) c.fail(`donation ticket price wrong: ${dt.price}`);
  c.examined(1);
  // And the donation counts against budget: too dear, not swept.
  const donBroke = planPurchases({ ...base, offers: [{ ...junk("J1", 10e6), repReq: 5e3, factionRep: 0, donationCost: 4e9 }], money: 2e9, ticketsWanted: 5 });
  if (donBroke.buy.some((b) => b.name === "J1")) c.fail("a donation ticket over budget must not be swept");

  c.examined(1);
  // Two factions selling the SAME ticket: one copy only — Daedalus counts
  // DISTINCT augmentations, and the first live sweep bought Neurotrainer II
  // twice because the dedup ran before the loop instead of inside it.
  const dup = planPurchases({
    ...base,
    offers: [{ ...junk("Twin", 10e6), faction: "A" }, { ...junk("Twin", 10e6), faction: "B" }],
    money: 1e12, ticketsWanted: 5,
  });
  if (dup.buy.filter((b) => b.name === "Twin").length !== 1) c.fail("the same aug from two factions must sweep in ONCE");

  c.examined(1);
  // THE RED PILL: m = 1, price $0 — must be TAKEN when reputation reaches,
  // and must not inflate the reported M (the gate spends real hours on M).
  const pill = { name: "The Red Pill", faction: "Daedalus", baseCost: 0, repReq: 2.5e6, factionRep: 2.6e6, mults: {}, prereqs: [] };
  const endgame = planPurchases({ ...base, offers: [real, pill], money: 1e12 });
  if (!endgame.buy.some((b) => b.name === "The Red Pill")) c.fail("the Red Pill with reputation reached MUST be in the plan");
  if (Math.abs(endgame.M - 1.5) > 1e-9) c.fail(`the synthetic selection value leaked into M: ${endgame.M} (expected 1.5)`);
  c.examined(1);
  // Rep-short Red Pill at a donatable faction: the donation machinery prices it.
  const pillWall = planPurchases({ ...base, offers: [{ ...pill, factionRep: 0, donationCost: 1.6e12 }], money: 2e12 });
  if (!pillWall.buy.some((b) => b.name === "The Red Pill" && b.donation > 0)) {
    c.fail("a donatable Red Pill must plan through the rep wall like any other aug");
  }
  return c;
}

/* ------------------------------------------------------------------ AP13 --- */
// NFG through donations: the planner sees what nfg.js's runtime was already
// buying. Before this, a fresh life (rep zeroed, real augs owned) planned
// only tickets, logM read 0, and the derived objective refused every early
// pass for want of a valued plan.
function ap13() {
  const c = new Check("AP13", "donatable NFG plans as a chain with per-level escalating donations");
  const nfg = {
    name: "NeuroFlux Governor", faction: "CyberSec", baseCost: 1e9, repReq: 40e3, factionRep: 0,
    donationCost: 32e9, // 40k short at $800k/rep
    mults: { hacking: 1.01, hacking_money: 1.01 }, prereqs: [], nfgLevel: 0,
  };
  const base = { r: 1.9, nodeMoneyMult: 1, owned: [], soaOwned: 0 };

  c.examined(1);
  // Rep-short WITHOUT a donation: blocked, exactly as before.
  const walled = planPurchases({ ...base, offers: [{ ...nfg, donationCost: undefined }], money: 1e15 });
  if (walled.buy.length !== 0 || !walled.skipped.some((x) => x.why === "reputation")) {
    c.fail("undonatable rep-short NFG must stay blocked");
  }

  c.examined(1);
  // WITH one: levels plan, each carrying its own escalated donation.
  const open = planPurchases({ ...base, offers: [nfg], money: 1e15 });
  const lv = open.buy.filter((b) => b.name === "NeuroFlux Governor");
  if (lv.length < 3) { c.fail(`expected several NFG levels at $1q, got ${lv.length}`); return c; }
  const perRep = 32e9 / 40e3;
  for (const [i, b] of lv.entries()) {
    const expectDon = Math.max(0, 40e3 * Math.pow(1.14, i) - 0) * perRep;
    if (Math.abs((b.donation ?? 0) - expectDon) > 1) { c.fail(`level ${i + 1} donation ${b.donation}, expected ${expectDon}`); break; }
  }
  c.examined(1);
  // The budget binds on intrinsic + donation together.
  const tight = planPurchases({ ...base, offers: [nfg], money: 34e9 }); // one level: 1e9 + 32e9 = 33e9 fits; level 2 doesn't
  const tl = tight.buy.filter((b) => b.name === "NeuroFlux Governor");
  if (tl.length !== 1) c.fail(`at $34b exactly one donated level fits, got ${tl.length}`);
  c.examined(1);
  // logM is REAL again on a fresh-life pass: the plan has value, so the
  // derived objective upstream gets a footing.
  if (!(open.logM > 0.02)) c.fail(`several NFG levels must carry real logM, got ${open.logM}`);
  return c;
}

export async function run() {
  return [ap1(), ap2(), ap3(), ap4(), ap5(), await ap6(), ap7(), ap8(), ap9(), ap10(), ap11(), ap12(), ap13()];
}
