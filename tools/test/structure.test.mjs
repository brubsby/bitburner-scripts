 ...
 *
 *   * healthcheck.mjs read `state.exp.hacking` and `state.lastAugReset`. The
 *     daemon digest published neither, so two movement checks evaluated null
 *     forever and skipped themselves while the tool printed HAPPY PATH.
 *   * go.js read `gate.goPower`, which /tel/installgate.txt has never carried.
 *     It defaulted to 1 and would have under-priced every Go opponent by 4x in
 *     BitNode 14 — the one node where Go is the whole point.
 *   * stage.test.mjs read `.telemetry/bn.txt`, a file with NO WRITER anywhere
 *     in this repo, frozen two BitNodes back. It answered, so it looked live.
 *
 * Every one failed the same way: silently, in the permissive direction, in
 * code whose job is to notice things. A read of an absent field is `undefined`,
 * `?? fallback` turns that into a plausible number, and nothing anywhere says
 * the input was missing.
 *
 * This checks the DECLARED contracts below: for each, the fields a reader
 * depends on must appear in the publisher's own source. It is deliberately a
 * short list of load-bearing pairs rather than a general dataflow analysis —
 * precise beats complete, per this file's header.
 *
 * WHAT IT DOES NOT CATCH: a field present in the publisher but never populated
 * at runtime, a field written only on some branches (the `weights` case below
 * is exactly that — it exists only on the DERIVED objective, which is
 * correct), and any reader not listed here.
 */
function c11() {
  const c = new Check("C11", "readers do not depend on fields their publisher never writes");
  const CONTRACTS = [
    {
      reader: "tools/healthcheck.mjs",
      publisher: "tools/rfa-daemon.mjs",
      what: "the daemon save digest",
      fields: ["money", "karma", "skills", "exp", "home", "augmentations", "playtimeSinceLastAug", "bitNode"],
      // SCOPED TO digest(), not the whole file. Searching all of
      // rfa-daemon.mjs passed over the deletion of `exp: p.exp` the first time
      // this was mutation-tested, because "exp" appears elsewhere in a 600-line
      // daemon. A contract check that matches anywhere in the publisher is not
      // checking the contract.
      scope: /function digest\(save\)[\s\S]*?\n\}/,
    },
    {
      reader: "go.js",
    ...
    },
    {
      ...
      fields: ["bitNode", "karmaPerSec", "killsPerSec", "contributing", "sleeves", "expToPlayerHacking", "factionRepPerSec", "disableSleeveExp"],
  ...
  for (const { reader, publisher, what, fields, scope } of CONTRACTS) {
    let pubSrc;
    try {
      pubSrc = read(publisher);
    } catch {
      c.fail(`${publisher} is not readable — cannot verify what ${reader} depends on`);
      continue;
    }
    // COMMENTS ARE NOT WRITES. Matching raw source passed over the deletion of
    // `exp: p.exp` twice: first because the search covered the whole 600-line
    // daemon, and then because the comment explaining the field says "exp"
    // three times. A check whose own documentation satisfies it is checking
    // nothing.
    pubSrc = pubSrc.replace(/\/\*[\s\S]*?\*\//g, " ").replace(/(^|[^:])\/\/[^\n]*/g, "$1 ");
    if (scope) {
      const m = pubSrc.match(scope);
      if (!m) {
        c.fail(`could not locate ${what} in ${publisher}`, "the contract cannot be checked, which is a rotted check rather than a clean repo");
        continue;
      }
      pubSrc = m[0];
    }
    ...
    c.note(`${reader} <- ${publisher}: ${fields.length} field(s) present in ${what}`);
  }

  // A telemetry file with no writer is worse than no file: it answers.
  //
  // COMMENTS ARE NOT READS. The first version of this matched the file name
  // anywhere in the source and so flagged the two test files that merely
  // DESCRIBE the bn.txt defect in prose — a false positive generated by the
  // fix's own documentation, and the kind that gets a check deleted. It now
  // matches a real readFileSync argument only.
  const READ_RE = /readFileSync\([^)]*bn\.txt/;
  const WRITE_RE = /ns\.write\(\s*['"`][^'"`]*bn\.txt/;
  c.examined(1);
  const readsBn = allFiles().filter((f) => /\.mjs$/.test(f) && READ_RE.test(read(f) ?? ""));
  const writesBn = rootScripts().filter((f) => WRITE_RE.test(read(f) ?? ""));
  if (readsBn.length && !writesBn.length) {
    c.fail(`${readsBn.join(", ")} read /tel/bn.txt, which NO root script writes`, "it was frozen two BitNodes back and still parsed, so it read as live");
  } else {
    c.note(`/tel/bn.txt: ${readsBn.length} reader(s), ${writesBn.length} writer(s)`);
  }
  return c;
}
