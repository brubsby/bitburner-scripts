...
const ASSUMPTIONS = [
  ...
  },
  {
    ...
    breaks:
      ...
      "The exit's capital return is the trader's REALISED return fitted across this node's lives (nodeecon.fitCapital: ln(end/start) = r(T - warm-up), from the lifetimes ledger's capStart/capEnd; progress.js capitalFitOf), not its instantaneous rate — a young life reads ~0 and priced installs every 35 min (live 21:46). Cash raises only for a join ready but for its cash (nodeecon.joinReadyButCash), and act.js releases the trader's hold as soon as the batch has run.",
  },
  {
    id: "hack-exp",
