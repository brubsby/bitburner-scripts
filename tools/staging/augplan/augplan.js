...
import { RATE_CHANNELS, progressFactor } from 'installgate.js'
...
export const valueOf = (mults, channels = RATE_CHANNELS) => progressFactor([mults], channels)
...
function normalise(o) {
  ...
  for (const r of repOk) {
    if (r.m > 1 + 1e-12) valuable.push(r)
    else skipped.push({ name: r.name, faction: r.faction, why: 'no-value', m: r.m, price: r.baseCost * nodeMoneyMult, channels })
  }
  ...
  if (nfgRow) {
    const repCap = nfgLevelsRepAllows(nfgRow.repReq, nfgRow.factionRep)
    const first = nfgRow.baseCost * Math.pow(NFG_LEVEL_MULT, nfgRow.nfgLevel) * nodeMoneyMult
    // A level whose intrinsic price alone exceeds the budget can never be taken
    // (its rank multiplier is >= 1), so the chain is bounded without a guess.
    let moneyCap = 0
    while (moneyCap < 64 && first * Math.pow(NFG_LEVEL_MULT, moneyCap) <= (o.money ?? 0)) moneyCap++
    ...
    for (const { st, f } of layer.values()) {
      ...
      for (const pt of f) {
        ...
        if (st.i < n) {
          ...
          const cost = pt.cost + it.intrinsic * Math.pow(r, st.rank)
          if (cost <= money + EPS) {
...
export function planPurchases(o) {
  ...
  for (const s of steps) {
    ...
    const price = isSoAStep ? s.it.intrinsic : s.it.intrinsic * Math.pow(r, rank)
    total += price
    buy.push({
      name: s.it.name,
 ...
 */
export function greedyByPrice({ offers, money, r = BASE_PRICE_MULT, nodeMoneyMult = 1, channels = RATE_CHANNELS, owned = [] }) {
  const have = new Set(owned)
  const rows = (offers ?? [])
    .filter((a) => (a.factionRep ?? 0) >= (a.repReq ?? 0))
    .map((a) => ({ ...a, price: (a.baseCost ?? a.price) * nodeMoneyMult, m: valueOf(a.mults ?? {}, channels) }))
