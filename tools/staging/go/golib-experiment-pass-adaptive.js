...
// --- Pass and time-management defaults -------------------------------------
//
// PASS_DEFAULT: whether PASS is allowed to WIN the root selection, rather than
// only being explored. See the selection block at the end of this function for
// why the answer is what it is and what the alternative cost.
//
// ADAPT_*: time management. A fixed budget spends as much on a move with one
// plausible answer as on the move that decides the game. `v1/(v1+v2)` over the
// two most-visited root children is scale-free (0.5 = tied, 1.0 = decided),
// unlike a share of total visits which moves with the branching factor.
const PASS_DEFAULT = false
const ADAPT_DEFAULT = false
const ADAPT_MINMS = 0.12
const ADAPT_CONFIDENCE = 0.8
// Minimum direct visits on the leader before its dominance means anything —
// 8 visits at 100% is noise, not confidence.
const ADAPT_MINVISITS = 30
// Passing on a coin-flip evaluation throws away a stone of tempo for nothing,
// and the two estimates are noisy, so PASS has to be strictly better by a
// margin AND be sampled enough for the margin to exist.
const PASS_MARGIN = 0.02
const PASS_MINVISITS = 30

export function chooseMoveUCT(boardStrings, valid, N, komi, maxms, opts) {
  const allowPass = opts?.allowPass ?? PASS_DEFAULT
  const adaptive = opts?.adaptive ?? ADAPT_DEFAULT
  const minms = opts?.minms ?? ADAPT_MINMS
  const confidence = opts?.confidence ?? ADAPT_CONFIDENCE
  const trace = opts?.trace ?? 0
  const traceLog = trace ? [] : null
  let lastTrace = 0
  const nbrs = makeGeometry(N)
  const root = parseBoard(boardStrings)
  const scratch = makeScratch(N)
  ...
  // minms is a FRACTION of the budget, not a constant: the point of adaptive
  // time is to spend a share of a budget, and a fixed floor in milliseconds
  // would mean something different at 200ms and at 1500ms.
  const floor = started + (minms <= 1 ? minms * maxms : minms)
  let iters = 0
  ...
  while (true) {
    ...
    if ((iters & 15) === 0) {
      const now = Date.now()
      // --- trace: what the search WOULD have answered, as a function of time.
      // Recorded at ~`trace` ms granularity so an early-exit rule can be scored
      // offline against the full-budget answer, instead of needing one live run
      // per candidate threshold. Off unless asked for; costs nothing when off.
      if (trace && now - lastTrace >= trace) {
        lastTrace = now
        let v1 = -1, v2 = -1, i1 = -1, c1 = null
        for (const [idx, child] of rootNode.children) {
          if (idx === PASS) continue
          if (child.visits > v1) { v2 = v1; v1 = child.visits; i1 = idx; c1 = child }
          else if (child.visits > v2) v2 = child.visits
        }
        const p = rootNode.children.get(PASS)
        traceLog.push({
          t: now - started, iters, idx: i1, v1, v2: Math.max(v2, 0),
          mean: c1 && c1.visits ? c1.winsUS / c1.visits : null,
          pv: p ? p.visits : 0,
          pmean: p && p.visits ? p.winsUS / p.visits : null,
        })
      }
      if (now >= deadline) break
      // --- adaptive time: stop once the root ordering is decided ------------
      if (adaptive && now >= floor && rootNode.untried.length === 0) {
        let v1 = -1, v2 = -1
        for (const [idx, child] of rootNode.children) {
          if (idx === PASS) continue
          if (child.visits > v1) { v2 = v1; v1 = child.visits }
          else if (child.visits > v2) v2 = child.visits
        }
        if (v1 >= ADAPT_MINVISITS && (v2 <= 0 || v1 / (v1 + v2) >= confidence)) break
      }
    }
  }
  ...
  for (const [idx, child] of rootNode.children) {
    if (idx === PASS) continue
    if (child.visits > bestVisits) { bestVisits = child.visits; bestIdx = idx; bestChild = child }
  }
  const elapsed = Date.now() - started

  // --- should we pass instead? ----------------------------------------------
  //
  // Returning [] rather than null so both existing consumers agree: go.js:271
  // tests `!ranked || !ranked.length` and tools/go-solver.mjs tests
  // `ranked && ranked.length`. Either sentinel passes; the empty array carries
  // the diagnostic fields with it, `null` cannot.
  //
  // Note what passing does NOT do. passTurn only ends the game at
  // passCount >= 2 (boardState.ts:146-157) and any makeMove resets passCount to
  // 0 (boardState.ts:136), while the AI passes only when it has no valid move
  // option at all (goAI.ts:198-218). So our pass ends the game only if the
  // opponent was already out of moves; otherwise it hands over a free move and
  // the game continues. Passing is therefore a decision about the position, not
  // a way to bank a lead, and it is evaluated as such.
  if (bestIdx === null) return []
  if (allowPass) {
    const p = rootNode.children.get(PASS)
    if (p && p.visits >= PASS_MINVISITS && bestChild && bestChild.visits > 0) {
      const passMean = p.winsUS / p.visits
      const moveMean = bestChild.winsUS / bestChild.visits
      if (passMean > moveMean + PASS_MARGIN) {
        return Object.assign([], { pass: true, passMean, moveMean, passVisits: p.visits, iters, elapsed, trace: traceLog })
      }
    }
  }
  return [{ x: (bestIdx / N) | 0, y: bestIdx % N, idx: bestIdx, visits: bestVisits, iters, elapsed,
    rootVisits: rootNode.visits, winRate: bestChild ? bestChild.winsUS / bestChild.visits : null,
    passVisits: rootNode.children.get(PASS)?.visits ?? 0,
    passMean: (() => { const p = rootNode.children.get(PASS); return p && p.visits ? p.winsUS / p.visits : null })(),
    trace: traceLog }]
}
