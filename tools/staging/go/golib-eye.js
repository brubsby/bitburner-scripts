...
/**
 * Is this point an eye for `colour`?
 *
 * The orthogonal-only test this replaces cannot tell a real eye from a FALSE
 * eye: a point whose four orthogonal neighbours are ours but whose diagonals
 * carry enemy stones belongs to a group that is not actually connected around
 * it, and is dead. Playouts refuse to fill "eyes", so a false eye makes a dead
 * group look alive for the whole search — it is never defended, and then it
 * dies all at once.
 *
 * Classic false-eye rule (the diagonal/Benson condition): an INTERIOR point
 * tolerates at most one hostile diagonal; an EDGE or CORNER point tolerates
 * none, because it has fewer diagonals to spare. Only enemy STONES are
 * hostile: empty diagonals are not yet anything, and an offline node (DEAD)
 * can never become an enemy stone, so both are counted as friendly here.
 * Treating empty diagonals as hostile would make the test so strict that
 * playouts fill real eyes and every group dies in simulation, which is a worse
 * bias than the one being fixed.
 */
function isOwnEye(b, nbrs, idx, colour, N) {
  ...
  for (let d = 0; d < 4; d++) {
    const nx = x + (d < 2 ? 1 : -1)
    const ny = y + (d % 2 ? 1 : -1)
    if (nx < 0 || ny < 0 || nx >= N || ny >= N) { offBoard++; continue }
    if (b[nx * N + ny] === enemy) hostile++
  }
  return offBoard ? hostile === 0 : hostile <= 1
}
...
export function chooseMoveUCT(boardStrings, valid, N, komi, maxms) {
  ...
  const legal = (b, colour) => {
    ...
    for (let i = 0; i < N * N; i++) {
      ...
      // Was an inline duplicate of the orthogonal-only eye test. It has to use
      // the same rule as the playouts, or the root excludes precisely the
      // points that the false-eye fix exists to let us consider.
      if (isOwnEye(b, nbrs, i, colour, N)) continue
      out.push(i)
    }
    return out
  }
