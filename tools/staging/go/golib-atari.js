  ...
  for (let turn = 0; turn < limit && passes < 2; turn++) {
    ...
    if (last >= 0) {
      ...
      for (let j = 0; j < ln.length; j++) {
        ...
        if (lib < 0) continue
        // Answer the LARGEST group in atari, not the first one found. Scanning
        // order is board order, so the old code would cheerfully rescue a lone
        // stone while a ten-stone group next to it was taken.
        if (v === colour) { if (gr.size > saveSize) { save = lib; saveSize = gr.size } }
        else if (gr.size > takeSize) { take = lib; takeSize = gr.size }
      }
