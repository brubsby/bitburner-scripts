...
export function heuristic(b, nbrs, idx, scratch) {
  ...
  // Danger scales with what is at stake. The function already scales its
  // UPSIDE by size — `rescued += before.size` and `atari += g.size` above — and
  // then prices its own risk as if every group were one stone: self-atari of a
  // 15-stone group scored exactly the same as self-atari of a lone stone, and
  // `4 * min(libs, 6)` rated a 12-stone group on two liberties the same as a
  // single stone on two. `mine.size` was computed on the line above and never
  // used. Confirmed on a real board (tools/staging/go/dissect.mjs): our single
  // 8-stone chain went 4 -> 3 liberties to the opponent, then WE played a stone
  // that took it from 3 to 2, then it was taken from 2 to 1, and all 8 died.
  if (mine.libs === 1 && captured === 0) return -500 - 50 * mine.size
  // Liberties are worth what they protect: a group wants roughly one liberty
  // per two stones before it is comfortable, so charge for the shortfall.
  const exposure = Math.max(0, mine.size - 2 * mine.libs)
  return 14 * rescued + 12 * captured + 6 * atari + 4 * Math.min(mine.libs, 6) - 6 * exposure
}
