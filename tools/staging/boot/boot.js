...
const STACK = [
  ...
  },
  {
    ...
    tier: 32,
    rank: 5,
    why: 'backdoors gate the faction invites that gate every augmentation; an install wipes them and forgetting it means reputation earns zero until a human notices. Needs cmd.js, so it is ranked directly after it',
  },
  {
    script: 'settings.js',
    where: 'home',
  ...
  },
  {
    ...
    tier: 64,
    rank: 11,
    why: 'buys the port programs. Its price is a function of the Source-File level — 5.85GB inside BitNode 4, 65.85GB at SF4.1 elsewhere (RamCostGenerator.ts:82-96) — so it is admitted on ns.getScriptRam, not on this tier alone, and on an SF4.1 save it will correctly sit deferred until home is large enough to hold 65.85GB',
  },
  {
    script: 'homeup.js',
    where: 'home',
  ...
  },
  {
    ...
    tier: 128,
    rank: 15,
    why: 'watchdog-triggered. Converts money into NeuroFlux levels through donations, which need 150 favour, which needs an install — so there is nothing for it to do in the first life at any home size',
  },
  {
    script: 'share.js',
    where: 'anywhere',
...
export async function main(ns) {
  ...
  for (const entry of plan.admit) {
    ...
    try {
      ...
      if (host !== 'home') ns.scp(entry.script, host, 'home')
