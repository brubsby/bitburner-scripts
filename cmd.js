...
export async function main(ns) {
  ...
  // WHAT RAN, not just how much. `batches` and `commandsRun` are counters, and
  // a counter can only ever say that something is happening — never what.
  //
  // Live cost of that gap: this bridge reached 1,994 batches and 2,040 commands
  // in one BitNode, every ~20s, all of them `run progress.js` failing for want
  // of 2.60GB. Every batch takes the GLOBAL UI LOCK, which is the same lock
  // autobuy.js needs to buy port programs and homeup.js needs for Alpha
  // Enterprises — so the churn was contending for the one resource the node's
  // progress depended on. Finding the writer meant polling /cmd/in.txt from
  // outside and hoping to catch a window narrower than the sampling interval,
  // which never worked, because the file is created and consumed inside one
  // synchronous turn.
  //
  // A ring of the last few batches costs nothing and answers it directly: the
  // command IS the signature of whoever queued it.
  const recent = []
  ...
  while (true) {
    try {
      ...
      try {
        ...
        recent.push({ at: new Date().toISOString(), cmds: queued.slice(0, 4) })
