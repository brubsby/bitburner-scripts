// STAGED ns.ramOverride REWRITE of faction.js.
//
// Generated by tools/staging/ramoverride/stage.mjs from the shipped faction.js.
...
//
// ---------------------------------------------------------------------------
// RAMOVERRIDE 2.6GB — excludes: ns.singularity.getAugmentationsFromFaction / getOwnedAugmentations / getAugmentationStats (5GB each at base, 80GB each at SF4.1), getAugmentationRepReq / getAugmentationPrice (2.5/40GB) and getCompanyRep (1/16GB).
//
...
const RAMOVERRIDE_STATUS = '/tel/faction.txt'
 ...
 *  SF4.3). 5.2 = everything not under ns.singularity; 21 = the
...
const RAISE_CEILING = (mult) => 5.2 + 21 * mult
...
export async function main(ns) {
  ...
  ns.atExit(() => note.exit('stopped', { detail: 'faction.js exited' }))
  ...
  if (!canUseSingularity(info)) {
    note('waiting', {
      ...
      detail:
        'canUseSingularity() is false for this save, so faction.js cannot act. Staying at the 2.6GB floor ' +
        'instead of reserving its full price. This script only PRINTS (--print-factions / --print-augs / --print-json). Without Source-File 4 the same information is in the Factions and Augmentations tabs.',
    })
    return
  }

  const want = RAISE_CEILING(singularityRamMultiplier(info))
  if (!(await raiseRam(ns, want, RAMOVERRIDE_STATUS, 'faction.js needs its full allocation before the first gated call'))) return

  try {
    note('ok', { result: 'running', allocation: want, detail: 'allocation raised; running the original body' })
    await act(ns)
    note('ok', { result: 'finished', detail: 'faction.js returned normally' })
  } catch (err) {
...
export const factions = {
  ...
  "Daedalus": {
    short_name: "Daedalus",
    requirements: {
      // ABSOLUTE COUNT, not a scale factor. The game passes this value straight
      // into haveAugmentations() (Faction/FactionInfo.tsx:142) and reads it as
      // `numAugReq` (Programs/Programs.ts:343) — it is the number of
      // augmentations required, full stop.
      //
      // This read `30 * ...`, which was right only by accident while the local
      // fallback table carried the WRONG default of 1. bitNodeMultipliers.js was
      // later corrected to the game's real default of 30, and this line
      // immediately began demanding 900 augmentations — visible in
      // `faction.js --print-factions` as `augmentations: 900.000` for Daedalus.
      // The header of bitNodeMultipliers.js predicted exactly this failure; the
      // table was fixed and its consumer was not.
      augmentations: readBitNodeMults().DaedalusAugsRequirement,
      money: 100000000000,
      hacking: 2500,
      strength: 1500,
      defense: 1500,
...
const areFactionRequirementsMet = (ns, factionEntry) => {
  ...
  if (requirements.hacking_combat_or) {
    if ((requirements.hacking && requirements.hacking > player.skills.hacking) &&
    ((requirements.strength && requirements.strength > player.skills.strength) ||
    (requirements.defense && requirements.defense > player.skills.defense) ||
    (requirements.dexterity && requirements.dexterity > player.skills.dexterity) ||
    (requirements.agility && requirements.agility > player.skills.agility))) {
      return false;
    }
  } else {
    if ((requirements.hacking && requirements.hacking > player.skills.hacking) ||
...
const getUnmetRequirements = (ns, factionName) => {
  ...
  // `hacking_combat_or` is a MODE FLAG (faction.js:111), not a requirement, and
  // it is only deleted inside the satisfied branch above. Any faction whose
  // combined gate is still unmet therefore carries a boolean into the report,
  // where ns.format.number(true, 3) throws:
  //
  //   TYPE ERROR  format.number: 'n' must be a number. Is of type 'boolean'
  //
  // The throw is inside the single tprint template, so --print-factions emitted
  // NOTHING — and watchdog.js added that flag precisely so this file could not
  // be silent. It also raises a modal, and a modal blocks the terminal, which
  // wedges cmd.js while it holds the global UI lock. Deleting it here leaves the
  // numeric requirements to speak for themselves.
  delete requirements.hacking_combat_or
  if (requirements.hacking && requirements.hacking <= player.skills.hacking)
    delete requirements.hacking;
  if (requirements.strength && requirements.strength <= player.skills.strength)
    delete requirements.strength;
  if (requirements.defense && requirements.defense <= player.skills.defense)
    delete requirements.defense;
  if (requirements.dexterity && requirements.dexterity <= player.skills.dexterity)
    delete requirements.dexterity;
  if (requirements.agility && requirements.agility <= player.skills.agility)
    delete requirements.agility;
  return requirements;
};
          ...
          `\n${requirementEntry[0].padStart(15)}: ${
            typeof(requirementEntry[1]) == "object" ?
              ...
              typeof(requirementEntry[1]) == "string" ?
                requirementEntry[1] :
                typeof requirementEntry[1] === "boolean" ?
                  String(requirementEntry[1]).padStart(8) :
