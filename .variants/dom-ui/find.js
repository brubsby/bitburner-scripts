...
export async function main(ns) {
  ...
  if (serverToFind) {
    if (Object.keys(serverMap.servers).includes(serverToFind)) {
      ns.tprint(`[${localeHHMMSS()}] Path to ${serverToFind} found: (Hack lvl: ${ns.getServerRequiredHackingLevel(serverToFind)})`)
    ...
    ns.tprint(`* CSEC (CyberSec faction) * Hack lvl: ${ns.getServerRequiredHackingLevel('CSEC')}`)
    ns.tprint(printPathToServer(serverMap.servers, 'CSEC') + '; hack;')
    ns.tprint('')
    ns.tprint(`* avmnite-02h (NiteSec faction) * Hack lvl: ${ns.getServerRequiredHackingLevel('avmnite-02h')}`)
    ns.tprint(printPathToServer(serverMap.servers, 'avmnite-02h') + '; hack;')
    ns.tprint('')
    ns.tprint(`* I.I.I.I (The Black Hand faction) * Hack lvl: ${ns.getServerRequiredHackingLevel('I.I.I.I')}`)
    ns.tprint(printPathToServer(serverMap.servers, 'I.I.I.I') + '; hack;')
    ns.tprint('')
    ns.tprint(`* run4theh111z (Bitrunners faction) * Hack lvl: ${ns.getServerRequiredHackingLevel('run4theh111z')}`)
    ns.tprint(printPathToServer(serverMap.servers, 'run4theh111z') + '; hack;')
    ns.tprint('')
    ns.tprint(`* w0r1d_d43m0n * Hack lvl: ${ns.getServerRequiredHackingLevel('w0r1d_d43m0n')}`)
    ...
    ns.tprint('home; connect darkweb; buy BruteSSH.exe; buy FTPCrack.exe; buy relaySMTP.exe; buy HTTPWorm.exe; buy SQLInject.exe; buy ServerProfiler.exe; buy AutoLink.exe; buy DeepscanV1.exe; buy DeepscanV2.exe; home;')
  }
}
