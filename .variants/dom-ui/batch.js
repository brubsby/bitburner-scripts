 ...
 */
export function planBatch(t, ram, maxRam) {
  const phi = t.phi
  const k = t.k
  ...
  while (true) {
    ...
    try {
          ...
          for (let n = 1; n <= cand.length; n++) {
            ...
            for (let i = 0; i < n; i++) {
              ...
              inc += Math.min(
                ...
                (share * p.money) / (p.gb * cand[i].t.hackTime * 4),
              )
            }
            if (inc > bestInc) {
              bestInc = inc
      ...
      try {
        ns.write(
          SETTINGS.statusFile,
          JSON.stringify({ at: new Date().toISOString(), health: 'error', errors: errors.slice(-5) }, null, 2),
          'w',
        )
        if (self !== 'home') ns.scp(SETTINGS.statusFile, 'home', self)
      } catch {
