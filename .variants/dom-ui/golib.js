...
export function heuristic(b, nbrs, idx, scratch) {
  ...
  probe.set(b)
  const captured = play(probe, nbrs, idx, US, scratch)
  if (captured < 0) return -1e9
  const mine = group(probe, nbrs, idx, out, seen, ++scratch.mark)
  ...
  for (let j = 0; j < ns.length; j++) {
    ...
    } else if (probe[p] === THEM) {
      const g = group(probe, nbrs, p, out, seen, ++scratch.mark)
